<?php
/**
 * Dedicated Mobile Health Insurance
...
$params = [$agentId];
$where = "WHERE h.agent_id = ?";

if (!empty($search)) {
    $where .= " AND (h.policy_number LIKE ? OR h.customer_name LIKE ? OR h.mobile_number LIKE ?)";
    $searchTerm = "%$search%";
    $params[] = $searchTerm;
    $params[] = $searchTerm;
    $params[] = $searchTerm;
...
<div class="mb-3">
    <h2 class="h5 font-weight-700 m-0 text-dark">Health Insurance Policies</h2>
    <p class="small text-muted m-0">Mobile policy management & tracking</p>
</div>
    ...
    <?php foreach ($policies as $pol): ?>
        <div class="mobile-card">
            <div class="mobile-card-header">
                ...
                <span class="badge bg-light text-dark border"><?php echo sanitize($pol['company_name'] ?? 'Health'); ?></span>
            </div>
            ...
            <div class="mobile-card-body p-3">
                ...
                <div class="d-flex align-items-center justify-content-between gap-2">
                    <?php if (!empty($pol['mobile_number'])): ?>
                        <a href="tel:<?php echo sanitize($pol['mobile_number']); ?>" class="btn btn-outline-secondary btn-sm touch-action-btn flex-grow-1">
                            <i class="fa-solid fa-phone me-1"></i> Call
                        </a>
                    <?php endif; ?>

                    <?php if (!empty($pol['whatsapp_number'])): ?>
                        <a href="https://wa.me/<?php echo preg_replace('/[^0-9]/', '', $pol['whatsapp_number']); ?>" target="_blank" class="btn btn-success btn-sm touch-action-btn flex-grow-1">
...
<?php else: ?>
    <div class="mobile-card p-4 text-center text-muted">
        ...
        <p class="m-0 font-weight-500">No health insurance policies found.</p>
    </div>
<?php endif; ?>
